...
{
    class PixelReaderWpf1 : IPixelReader
    {
        public string Name { get; } = "PixelReaderWpf1(Wpf0+unsafe)";

        private readonly string ImagePath;

        public PixelReaderWpf1(string imagePath)
        {
            ImagePath = imagePath;
        }
            ...
            GetAverage(byte[] pixels, int width, int height, int pixelsByte)
        {
            ...
            unsafe
            {
                // byte[]をbyte*にするためにfixedが必要。unsafeにしても別に早くない気がした…
                fixed (byte* head = pixels)
                {
                    var tail = head + pixels.Length;
                    for (var p = head; p < tail; p += pixelsByte)
                    {
                        sumB += *p;
                        sumG += *(p + 1);
                        sumR += *(p + 2);
